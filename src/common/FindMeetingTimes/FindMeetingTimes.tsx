...
export class FindMeetingTimes extends React.Component<IFindMeetingTimesProps, IFindMeetingTimesState> {
  ...
  private _saveDialog = (): void => {
    let data ={ "attendees": this.getAttendees(),  
        "timeConstraint": {
          "activityDomain":"unrestricted", 
          "timeslots": [ 
            { 
              "start": { 
                "dateTime": this.getDate('start'),  
                "timeZone": "Eastern Standard Time"
              },  
              "end": { 
                "dateTime":this.getDate('end'),  
                "timeZone": "Eastern Standard Time"
              } 
            } 
          ] 
        },  
        "meetingDuration": this.getMeetingDuration(),
        "returnSuggestionReasons": true,
        minimumAttendeePercentage:1.0,
        maxCandidates:50
      }
    this.eventsApi.FindMeetingTimes(this.props.token,data).then(res=>{
      console.log(res);
      this.setState(prevState => {
  ...
  private getAttendees():any[]{
    let result = [];
    if(this.state.location.key)
      result.push({
        "type": "required",  
        "emailAddress": { 
          "address": this.state.location.key
        }
      });
    else{
